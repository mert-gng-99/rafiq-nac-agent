...
1. Routine check, pilgrim with the group, the first scenario on the left. Watch the agent pick
...
2. Run all seven checks runs all 7 scenarios back to back and shows the
...
```bash
docker build -t rafiq . && docker run -p 8000:8000 rafiq
```
