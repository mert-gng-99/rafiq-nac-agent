# Rafiq - Prototype Phase submission
...
**YOU.** Pick your Idea Phase submission for Rafiq from the dropdown.
...
```
3. Tourism, Pilgrimage & Cultural Experience Innovation
```
...
1. The dashboard after Run all seven checks, showing the batch table and the cost comparison.
...
7 CAMARA APIs, 67.9% cheaper than calling every check.
...
```
https://github.com/mert-gng-99/rafiq-nac-agent
```
...
| --- | --- | --- |
| Parent Submission | Pick Rafiq from the dropdown | seconds |
...
| --- | --- |
| At least one CAMARA API on Nokia NaC | 7 API families: `number-verification`, `device-status`, `location-verification`, `location-retrieval`, `congestion-insights`, `geofencing-subscriptions`, `quality-on-demand` |
...
| Solves a real problem | Tourism, Pilgrimage & Cultural Experience Innovation. Stated limits are in README.md, so this is not pitched as doing more than it does. |
| Theme alignment | Theme 3 |
| Multiple CAMARA APIs (good-to-have) | 7 families orchestrated per case |
| Intelligent orchestration (good-to-have) | 1 to 6 calls depending on the case; 67.9% cheaper than calling everything |
